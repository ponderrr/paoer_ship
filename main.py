...
try:
    ...
    class DummyDisplayHandler:
        ...
        def draw_board_on_portable(self, *args, **kwargs):
            pass
            
        def draw_waiting_screen(self, *args, **kwargs):
            pass
...
def initialize_dual_screens():
    ...
    try:
        ...
        # Create a dummy portable screen by default
        portable_screen = pygame.Surface((480, 320))
        logger.debug("Created fallback surface for portable display")
...
def game_screen(ai_mode=True, difficulty="Medium", player1_board=None, player2_board=None):
    """Game screen stub for debugging"""
    logger.debug(f"Entering game_screen with ai_mode={ai_mode}, difficulty={difficulty}")
    # This is a simplified version just for testing
    try:
        screen.fill(BLACK)
        font = pygame.font.Font(None, 36)
        text = font.render(f"Game Mode: {'AI' if ai_mode else 'Player'} - {difficulty if ai_mode else ''}", True, WHITE)
        screen.blit(text, (WIDTH//2 - 180, HEIGHT//2))
        ...
        # Wait for a key press or button press
        waiting = True
        while waiting:
            for event in pygame.event.get():
                if event.type in (pygame.KEYDOWN, pygame.MOUSEBUTTONDOWN):
            ...
            # Small delay to prevent CPU hogging
            time.sleep(0.01)
        logger.debug("Exiting game_screen")
    except Exception as e:
