# import pygame
# import sys
# import time
# import numpy as np
# import random
# from src.board.game_board import GameBoard, CellState
# from ship_placement_screen import ShipPlacementScreen
# from src.game.ai_opponent import AIOpponent, AIDifficulty
# from src.utils.image_display import ImageDisplay

# # Try to import GPIO support
# try:
#     import gpiod
#     IS_RASPBERRY_PI = True
# except ImportError:
#     IS_RASPBERRY_PI = False

# # Initialize Pygame
# pygame.init()

# # Screen settings - smaller for better performance
# WIDTH, HEIGHT = 640, 480
# screen = pygame.display.set_mode((WIDTH, HEIGHT))
# pygame.display.set_caption("Pao'er Ship")

# # Colors
# WHITE = (255, 255, 255)
# BLACK = (0, 0, 0)
# BLUE = (50, 150, 255)
# LIGHT_BLUE = (80, 170, 255)
# LIGHT_GRAY = (180, 180, 180)

# # Load fonts
# pygame.font.init()
# title_font = pygame.font.Font(None, 50)
# button_font = pygame.font.Font(None, 30)

# class GPIOHandler:
#     def __init__(self):
#         self.chip = None
#         self.lines = {}
#         self.last_states = {
#             'up': False,
#             'down': False,
#             'left': False,
#             'right': False,
#             'fire': False,
#             'mode': False,
#             'rotate': False  # Add rotate button state
#         }
        
#         # Define GPIO pins for buttons (BCM numbering)
#         self.PIN_UP = 17    # Pin 11
#         self.PIN_DOWN = 27  # Pin 13
#         self.PIN_LEFT = 22  # Pin 15
#         self.PIN_RIGHT = 23 # Pin 16
#         self.PIN_FIRE = 24  # Pin 18
#         self.PIN_MODE = 25  # Pin 22
#         self.PIN_ROTATE = 26  # Pin 37 (TODO: Update this when the hardware is connected)
        
#         if IS_RASPBERRY_PI:
#             self.setup()
    
#     def setup(self):
#         try:
#             # Try to open the GPIO chip for Pi 5
#             self.chip = gpiod.Chip("gpiochip4")
            
#             # Pin to button name mapping
#             pin_button_map = {
#                 self.PIN_UP: 'up',
#                 self.PIN_DOWN: 'down',
#                 self.PIN_LEFT: 'left',
#                 self.PIN_RIGHT: 'right',
#                 self.PIN_FIRE: 'fire',
#                 self.PIN_MODE: 'mode',
#                 self.PIN_ROTATE: 'rotate'  # Add the new rotate button
#             }
            
#             # Set up all the lines using the older API (which we know works)
#             for pin, button_name in pin_button_map.items():
#                 line = self.chip.get_line(pin)
#                 line.request(consumer=f"paoer-ship-{button_name}", type=gpiod.LINE_REQ_DIR_IN)
#                 self.lines[pin] = line
                
#         except Exception as e:
#             print(f"Error setting up GPIO: {e}")
#             if self.chip:
#                 self.chip.close()
#                 self.chip = None
    
#     def cleanup(self):
#         if self.chip:
#             self.chip.close()
#             self.chip = None
    
#     def get_button_states(self):
#         actions = {
#             'up': False,
#             'down': False, 
#             'left': False,
#             'right': False,
#             'fire': False,
#             'mode': False,
#             'rotate': False  # Include rotate in actions
#         }
        
#         if not IS_RASPBERRY_PI or not self.chip:
#             return actions
        
#         try:
#             # Pin to button name mapping
#             pin_button_map = {
#                 self.PIN_UP: 'up',
#                 self.PIN_DOWN: 'down',
#                 self.PIN_LEFT: 'left',
#                 self.PIN_RIGHT: 'right',
#                 self.PIN_FIRE: 'fire',
#                 self.PIN_MODE: 'mode',
#                 self.PIN_ROTATE: 'rotate'  # Add the new rotate button
#             }
            
#             for pin, button_name in pin_button_map.items():
#                 if pin not in self.lines:
#                     continue
                
#                 # Read line value (active LOW with pull-up)
#                 line = self.lines[pin]
#                 # For buttons with pull-up resistors, 0 means pressed (active low)
#                 current_state = (line.get_value() == 0)
                
#                 # Only register a press when the state changes from released to pressed
#                 if current_state and not self.last_states[button_name]:
#                     actions[button_name] = True
                
#                 # Update last state
#                 self.last_states[button_name] = current_state
                
#         except Exception as e:
#             print(f"Error reading GPIO: {e}")
        
#         return actions

# # Simple button class
# class Button:
#     def __init__(self, x, y, width, height, text, action):
#         self.rect = pygame.Rect(x, y, width, height)
#         self.text = text
#         self.action = action
#         self.base_color = BLUE
#         self.hover_color = LIGHT_BLUE
#         self.current_color = self.base_color
#         self.hovered = False
#         self.selected = False

#     def update(self):
#         self.current_color = self.hover_color if (self.selected or self.hovered) else self.base_color

#     def draw(self, screen):
#         pygame.draw.rect(screen, self.current_color, self.rect, border_radius=5)
#         text_surface = button_font.render(self.text, True, WHITE)
#         text_rect = text_surface.get_rect(center=self.rect.center)
#         screen.blit(text_surface, text_rect)

#     def check_hover(self, pos):
#         self.hovered = self.rect.collidepoint(pos)

#     def check_click(self, pos):
#         if self.rect.collidepoint(pos):
#             self.action()

# # Forward declarations for functions that reference each other
# def quit_game():
#     gpio_handler.cleanup()
#     pygame.quit()
#     sys.exit()

# def settings_screen():
#     """Simplified settings screen"""
#     screen.fill(BLACK)
#     font = pygame.font.Font(None, 36)
#     text = font.render("Settings Screen (Placeholder)", True, WHITE)
#     screen.blit(text, (WIDTH//2 - 180, HEIGHT//2))
    
#     back_text = font.render("Press any key to return to menu", True, WHITE)
#     screen.blit(back_text, (WIDTH//2 - 180, HEIGHT//2 + 50))
    
#     pygame.display.flip()
    
#     waiting = True
#     while waiting:
#         for event in pygame.event.get():
#             if event.type in (pygame.KEYDOWN, pygame.MOUSEBUTTONDOWN):
#                 waiting = False
#             elif event.type == pygame.QUIT:
#                 pygame.quit()
#                 sys.exit()
        
#         # Check for GPIO button presses
#         button_states = gpio_handler.get_button_states()
#         if any(button_states.values()):
#             waiting = False
        
#         # Small delay to prevent CPU hogging
#         time.sleep(0.01)

# def process_shot(x, y, shooter_board, target_board, shots_set):
#     """Process a shot from one player to another's board"""
#     # Swap coordinates for internal board representation
#     # This translates display coordinates to board coordinates
#     # x = column (A-J), y = row (1-10)
#     # But internally board is accessed as [row][col]
#     board_x, board_y = y, x
    
#     if (board_x, board_y) in shots_set:
#         return False, False  # Shot already taken, no ship sunk
        
#     shots_set.add((board_x, board_y))
    
#     # Check if hit and if ship was sunk
#     hit = False
#     ship_sunk = False
    
#     for ship in target_board.ships:
#         if ship.receive_hit(board_x, board_y):
#             hit = True
#             ship_sunk = ship.is_sunk()
#             break
            
#     # Update the board state at the target location
#     if hit:
#         target_board.board[board_x, board_y] = CellState.HIT.value
#     else:
#         target_board.board[board_x, board_y] = CellState.MISS.value
            
#     return hit, ship_sunk

# def game_screen(ai_mode=True, difficulty="Medium", player1_board=None, player2_board=None):
#     """
#     Game screen where the actual gameplay happens after ship placement
    
#     Args:
#         ai_mode (bool): Whether playing against AI (True) or another player (False)
#         difficulty (str): AI difficulty level if ai_mode is True
#         player1_board: Game board for player 1
#         player2_board: Game board for player 2 (or AI)
#     """
#     try:
#         print(f"Starting game with AI mode: {ai_mode}, difficulty: {difficulty}")
        
#         # Import necessary modules
#         import numpy as np
#         import random
#         from turn_transition_screen import TurnTransitionScreen
#         from exit_confirmation import ExitConfirmation
#         from src.game.ai_opponent import AIOpponent, AIDifficulty
#         from src.utils.image_display import ImageDisplay
        
#         # Initialize game components
#         clock = pygame.time.Clock()
#         font = pygame.font.Font(None, 36)
#         small_font = pygame.font.Font(None, 24)
        
#         # Create empty boards if none were provided
#         if player1_board is None:
#             player1_board = GameBoard()
#         if player2_board is None:
#             player2_board = GameBoard()
        
#         # Initialize transition screen and exit confirmation dialogs
#         transition_screen = TurnTransitionScreen(screen, gpio_handler)
#         exit_dialog = ExitConfirmation(screen, gpio_handler)
#         image_display = ImageDisplay(screen)
        
#         # Define helper function for game over check
#         def check_game_over():
#             """Check if the game is over (all ships sunk)"""
#             player1_lost = all(ship.is_sunk() for ship in player1_board.ships)
#             player2_lost = all(ship.is_sunk() for ship in player2_board.ships)
            
#             if player1_lost:
#                 return 2  # Player 2 (or AI) wins
#             elif player2_lost:
#                 return 1  # Player 1 wins
#             return None  # No winner yet
        
#         # Set up AI opponent if in AI mode
#         ai_opponent = None
#         if ai_mode:
#             # Map difficulty string to enum
#             difficulty_map = {
#                 "Easy": AIDifficulty.EASY,
#                 "Medium": AIDifficulty.MEDIUM,
#                 "Hard": AIDifficulty.HARD,
#                 "Pao": AIDifficulty.PAO
#             }
            
#             ai_difficulty = difficulty_map.get(difficulty, AIDifficulty.MEDIUM)
#             ai_opponent = AIOpponent(ai_difficulty, player1_board)
            
#             # Use the AI's board as player 2's board
#             player2_board = ai_opponent.board
        
#         # Set up game mode specific variables
#         game_mode_text = "VS AI" if ai_mode else "VS Player"
#         if ai_mode and difficulty:
#             game_mode_text += f" ({difficulty})"
#         pao_mode = ai_mode and difficulty == "Pao"
        
#         # Game state variables
#         current_player = 1  # 1 for player 1, 2 for player 2 (or AI)
#         cursor_x, cursor_y = 0, 0
#         player1_shots = set()  # Track player 1's shots (x, y)
#         player2_shots = set()  # Track player 2's shots (x, y)
#         winner = None  # None, 1, or 2
#         move_delay = 0  # Delay for cursor movement
        
#         # Display boards for each player (what they can see)
#         player1_view = np.zeros((10, 10), dtype=int)
#         player2_view = np.zeros((10, 10), dtype=int)
        
#         # Get player's board state to display their own ships
#         player1_own_view = player1_board.get_display_state()
#         player2_own_view = player2_board.get_display_state()
        
#         # Show initial player screen if in player vs player mode
#         if not ai_mode:
#             transition_screen.show_player_ready_screen(current_player)
#         else:
#     # Show player ready screen with board in AI mode
#             transition_screen.show_player_ready_screen(
#                 current_player,    # Player 1
#                 True,              # is_ai_mode=True
#                 player1_own_view   # Player's board for preview
#             )
        
#         # Wait for next player message if two-player mode
#         current_message = ""
#         showing_message = False
#         message_timer = 0
#         showing_exit_dialog = False
#         pao_missed = False  # Track if player missed in Pao mode
        
#         # Main game loop
#         running = True
#         while running:
#             current_time = pygame.time.get_ticks()
            
#             # Fill background
#             screen.fill(BLACK)
            
#             # Draw game mode
#             mode_text = font.render(game_mode_text, True, WHITE)
#             screen.blit(mode_text, (WIDTH - 200, 20))
            
#             # Draw exit option
#             exit_text = small_font.render("Press MODE to exit", True, LIGHT_GRAY)
#             screen.blit(exit_text, (20, HEIGHT - 30))
            
#             if showing_exit_dialog:
#                 if exit_dialog.show():
#                     running = False  # Exit to main menu
#                     break  # Exit the loop immediately
#                 showing_exit_dialog = False
#                 continue  # Skip the rest of the loop iteration after dialog
            
#             # Determine whose turn it is and which board to display
#             if current_player == 1:
#                 # Player 1's turn - show player 2's board with player 1's shots
#                 active_board = player2_board
#                 view_board = player1_view
#                 shots = player1_shots
                
#                 # Also show player 1's board with their ships
#                 own_board = player1_own_view
#             else:
#                 # Player 2's turn - show player 1's board with player 2's shots
#                 active_board = player1_board
#                 view_board = player2_view
#                 shots = player2_shots
                
#                 # Also show player 2's board with their ships
#                 own_board = player2_own_view
            
#             # Draw opponent board (with shots but not ships)
#             # This is the board the current player is firing at
#             draw_board(
#                 screen, 
#                 font, 
#                 view_board, 
#                 150, 
#                 80, 
#                 30, 
#                 cursor_x if (current_player == 1 or (current_player == 2 and not ai_mode)) else -1,  # Show cursor for human players
#                 cursor_y if (current_player == 1 or (current_player == 2 and not ai_mode)) else -1,
#                 (current_player == 1 or (current_player == 2 and not ai_mode)),  # Show cursor for human players
#                 "Opponent's Board"
#             )
            
#             # Only show own board during AI mode
#             if ai_mode:
#                 # Draw player's own board (with ships) in AI mode
                
            
#             # Draw current player
#                 if not winner:
#                     if current_player == 1:
#                         player_text = font.render("Player 1's Turn", True, WHITE)
#                     elif not ai_mode:
#                         player_text = font.render("Player 2's Turn", True, WHITE)
#                 else:
#                     player_text = font.render("AI's Turn", True, WHITE)
#                     screen.blit(player_text, (20, 20))
            
#             # Draw status message
#             if winner:
#                 if winner == 1:
#                     winner_text = font.render("Player 1 Wins!", True, (255, 215, 0))
#                 elif ai_mode:
#                     winner_text = font.render("AI Wins!", True, (255, 0, 0))
#                 else:
#                     winner_text = font.render(f"Player {winner} Wins!", True, (255, 215, 0))
                
#                 screen.blit(winner_text, (WIDTH // 2 - 100, HEIGHT - 100))
                
#                 restart_text = small_font.render("Press MODE to return to menu", True, WHITE)
#                 screen.blit(restart_text, (WIDTH // 2 - 120, HEIGHT - 60))
#             else:
#                 # Status messages - use different text for AI vs human player 2
#                 if current_player == 1:
#                     status_text = small_font.render("Press FIRE to shoot", True, WHITE)
#                     screen.blit(status_text, (WIDTH // 2 - 80, HEIGHT - 40))
#                 else:
#                     # Different message based on if it's AI or human player 2
#                     if ai_mode:
#                         status_text = small_font.render("AI is thinking...", True, WHITE)
#                     else:
#                         status_text = small_font.render("Player 2: Press FIRE to shoot", True, WHITE)
#                     screen.blit(status_text, (WIDTH // 2 - 100, HEIGHT - 40))
            
#             # Process events
#             for event in pygame.event.get():
#                 if event.type == pygame.QUIT:
#                     running = False
#                     pygame.quit()
#                     sys.exit()
                    
#                 elif event.type == pygame.KEYDOWN and not showing_message:
#                     if event.key == pygame.K_ESCAPE or event.key == pygame.K_TAB:
#                         showing_exit_dialog = True
                    
#                     # Allow input for both players in non-AI mode, or just player 1 in AI mode
#                     if not winner and (current_player == 1 or (current_player == 2 and not ai_mode)):
#                         if event.key == pygame.K_UP and cursor_y > 0:
#                             cursor_y -= 1
#                         elif event.key == pygame.K_DOWN and cursor_y < 9:
#                             cursor_y += 1
#                         elif event.key == pygame.K_LEFT and cursor_x > 0:
#                             cursor_x -= 1
#                         elif event.key == pygame.K_RIGHT and cursor_x < 9:
#                             cursor_x += 1
#                         elif event.key == pygame.K_SPACE:
#                             # Player 1 fires
#                             if current_player == 1 and (cursor_y, cursor_x) not in player1_shots:  # Note the coordinate swap!
#                                 hit, ship_sunk = process_shot(cursor_x, cursor_y, None, player2_board, player1_shots)
                                
#                                 # Update the view - using correct coordinate mapping
#                                 if hit:
#                                     player1_view[cursor_y][cursor_x] = CellState.HIT.value  # Swap coordinates for view update
#                                 else:
#                                     player1_view[cursor_y][cursor_x] = CellState.MISS.value  # Swap coordinates for view update
                                    
#                                     # For Pao mode, player immediately loses if they miss
#                                     if pao_mode:
#                                         # Show miss before ending
#                                         screen.fill(BLACK)
#                                         draw_board(screen, font, player1_view, 150, 80, 30, cursor_x, cursor_y, True, "Your Shot")
#                                         draw_board(screen, font, player1_own_view, 400, 80, 25, -1, -1, False, "Your Board")
                                        
#                                         miss_text = small_font.render(f"You fired at {chr(65 + cursor_x)}{cursor_y + 1}: MISS!", True, (0, 0, 255))
#                                         screen.blit(miss_text, (WIDTH // 2 - 100, HEIGHT - 70))
                                        
#                                         pao_warning = font.render("PAO MODE ACTIVATED - YOU LOSE!", True, (255, 0, 0))
#                                         screen.blit(pao_warning, (WIDTH // 2 - 200, HEIGHT - 40))
                                        
#                                         pygame.display.flip()
#                                         pygame.time.delay(3000)
                                        
#                                         # Display Pao image
#                                         pao_missed = True
#                                         winner = 2  # AI wins
#                                         image_display.display_pao_image()
#                                         continue
                                
#                                 # Draw the updated board to show shot result before transition
#                                 pygame.display.flip()
                                
#                                 # Check if game over
#                                 new_winner = check_game_over()
#                                 if new_winner:
#                                     winner = new_winner
#                                     # Show the final shot before ending
#                                     pygame.display.flip()
#                                     # Wait a moment to show the winning shot
#                                     time.sleep(1)
#                                 else:
#                                     # Show the shot result
#                                     if not ai_mode:
#     # Show transition screens in player vs player mode
#                                         transition_screen.show_turn_result(current_player, cursor_y, cursor_x, hit, ship_sunk)
#                                         transition_screen.show_player_ready_screen(2)
#                                     else:
#     # Show transition screens in AI mode with board preview
#                                         transition_screen.show_turn_result(
#                                             current_player,  # Player 1
#                                             cursor_y,        # Row (swapped from cursor_y)
#                                             cursor_x,        # Column (swapped from cursor_x)
#                                             hit,
#                                             ship_sunk,
#                                             True,            # is_ai_mode=True
#                                             player1_own_view # Player's board for preview
#                                         )
#                                         transition_screen.show_player_ready_screen(
#                                             2,               # Next player (AI)
#                                             True,            # is_ai_mode=True
#                                             player1_own_view # Player's board for preview
#                                         )
                                    
#                                     # Switch to next player's turn
#                                     current_player = 2
                            
#                             # Player 2 fires (only in non-AI mode)
#                             elif current_player == 2 and not ai_mode and (cursor_y, cursor_x) not in player2_shots:  # Note the coordinate swap!
#                                 hit, ship_sunk = process_shot(cursor_x, cursor_y, None, player1_board, player2_shots)
                                
#                                 # Update the view - using correct coordinate mapping
#                                 if hit:
#                                     player2_view[cursor_y][cursor_x] = CellState.HIT.value  # Swap coordinates for view update
#                                 else:
#                                     player2_view[cursor_y][cursor_x] = CellState.MISS.value  # Swap coordinates for view update
                                
#                                 # Draw the updated board to show shot result before transition
#                                 pygame.display.flip()
                                
#                                 # Check if game over
#                                 new_winner = check_game_over()
#                                 if new_winner:
#                                     winner = new_winner
#                                     # Show the final shot before ending
#                                     pygame.display.flip()
#                                     # Wait a moment to show the winning shot
#                                     time.sleep(1)
#                                 else:
#                                     # Show the shot result
#                                     # Fixed coordinates for display - letter/number format
#                                     transition_screen.show_turn_result(current_player, cursor_x, cursor_y, hit, ship_sunk)
#                                     transition_screen.show_player_ready_screen(1)
                                    
#                                     # Switch back to player 1's turn
#                                     current_player = 1
            
#             # Check GPIO buttons if not showing message
#             if not showing_message:
#                 button_states = gpio_handler.get_button_states()
                
#                 # Check for exit button
#                 if button_states['mode']:
#                     showing_exit_dialog = True
                
#                 # Only process gameplay if no win condition yet and it's a human player's turn
#                 if not winner and (current_player == 1 or (current_player == 2 and not ai_mode)):
#                     if button_states['up'] and cursor_y > 0:
#                         cursor_y -= 1
#                     if button_states['down'] and cursor_y < 9:
#                         cursor_y += 1
#                     if button_states['left'] and cursor_x > 0:
#                         cursor_x -= 1
#                     if button_states['right'] and cursor_x < 9:
#                         cursor_x += 1
                        
#                     if button_states['fire']:
#                         # Player 1 fires
#                         if current_player == 1 and (cursor_y, cursor_x) not in player1_shots:  # Note the coordinate swap!
#                             hit, ship_sunk = process_shot(cursor_x, cursor_y, None, player2_board, player1_shots)
                            
#                             # Update the view - using correct coordinate mapping
#                             if hit:
#                                 player1_view[cursor_y][cursor_x] = CellState.HIT.value  # Swap coordinates for view update
#                             else:
#                                 player1_view[cursor_y][cursor_x] = CellState.MISS.value  # Swap coordinates for view update
                                
#                                 # For Pao mode, player immediately loses if they miss
#                                 if pao_mode:
#     # Show miss using transition screen
#                                  transition_screen.show_turn_result(
#                                      current_player,    # Player 1
#                                      cursor_y,          # Row
#                                      cursor_x,          # Column 
#                                      False,             # hit=False (it's a miss)
#                                      False,             # ship_sunk=False
#                                      True,              # is_ai_mode=True
#                                      player1_own_view   # Player's board for preview
#                                  )

#                                  # Show Pao mode activated message
#                                  screen.fill(BLACK)
#                                  pao_warning = font.render("PAO MODE ACTIVATED - YOU LOSE!", True, (255, 0, 0))
#                                  warning_rect = pao_warning.get_rect(center=(WIDTH // 2, HEIGHT // 2))
#                                  screen.blit(pao_warning, warning_rect)
#                                  pygame.display.flip()
#                                  pygame.time.delay(3000)

#                                  # Display Pao image
#                                  pao_missed = True
#                                  winner = 2  # AI wins
#                                  image_display.display_pao_image()
#                                  continue

                            
#                             # Draw the updated board to show shot result before transition
#                             pygame.display.flip()
                            
#                             # Check if game over
#                             new_winner = check_game_over()
#                             if new_winner:
#                                 winner = new_winner
#                                 # Show the final shot before ending
#                                 pygame.display.flip()
#                                 # Wait a moment to show the winning shot
#                                 time.sleep(1)
#                             else:
#                                 # Show the shot result
#                                if not ai_mode:
#     # Show transition screens in player vs player mode
#                                  transition_screen.show_turn_result(current_player, cursor_y, cursor_x, hit, ship_sunk)
#                                  transition_screen.show_player_ready_screen(2)
#                         else:
#                                 # Show transition screens in AI mode with board preview
#                                 transition_screen.show_turn_result(
#                                     current_player,    # Player 1
#                                     cursor_y,          # Row
#                                     cursor_x,          # Column
#                                     hit,
#                                     ship_sunk,
#                                     True,              # is_ai_mode=True
#                                     player1_own_view   # Player's board for preview
#                                 )
#                                 transition_screen.show_player_ready_screen(
#                                     2,                 # Next player (AI)
#                                     True,              # is_ai_mode=True
#                                     player1_own_view   # Player's board for preview
#                                 )

                                
#                                 # Switch to next player's turn
#                                 current_player = 2
                                
#                         # Player 2 fires (only in non-AI mode)
#                     elif current_player == 2 and not ai_mode and (cursor_y, cursor_x) not in player2_shots:  # Note the coordinate swap!
#                             hit, ship_sunk = process_shot(cursor_x, cursor_y, None, player1_board, player2_shots)
                            
#                             # Update the view - using correct coordinate mapping
#                             if hit:
#                                 player2_view[cursor_y][cursor_x] = CellState.HIT.value  # Swap coordinates for view update
#                             else:
#                                 player2_view[cursor_y][cursor_x] = CellState.MISS.value  # Swap coordinates for view update
                            
#                             # Draw the updated board to show shot result before transition
#                             pygame.display.flip()
                            
#                             # Check if game over
#                             new_winner = check_game_over()
#                             if new_winner:
#                                 winner = new_winner
#                                 # Show the final shot before ending
#                                 pygame.display.flip()
#                                 # Wait a moment to show the winning shot
#                                 time.sleep(1)
#                             else:
#                                 # Show the shot result
#                                 # Fixed coordinates for display - letter/number format
#                                 transition_screen.show_turn_result(current_player, cursor_x, cursor_y, hit, ship_sunk)
#                                 transition_screen.show_player_ready_screen(1)
                                
#                                 # Switch back to player 1's turn
#                                 current_player = 1
#                 elif winner:
#                     # If there's a winner, pressing any button will return to menu
#                     if any(button_states.values()):
#                         running = False  # Return to main menu
            
#             # AI logic - if it's the AI's turn
#             if ai_mode and current_player == 2 and not winner:
#                 # Add a delay to make the AI feel more human-like
#                 if difficulty == "Easy":
#                     thinking_time = random.uniform(0.5, 1.0)
#                 elif difficulty == "Medium": 
#                     thinking_time = random.uniform(1.0, 1.5)
#                 else:  # Hard or Pao
#                     thinking_time = random.uniform(1.5, 2.0)
                
#                 # Show thinking message
#                 screen.fill(BLACK)
#                 draw_board(screen, font, player2_view, 150, 80, 30, -1, -1, False, "AI's Shot")
#                 draw_board(screen, font, player1_own_view, 400, 80, 25, -1, -1, False, "Your Board")
#                 thinking_text = small_font.render("AI is thinking...", True, WHITE)
#                 thinking_rect = thinking_text.get_rect(center=(WIDTH // 2, HEIGHT - 40))
#                 screen.blit(thinking_text, thinking_rect)
#                 pygame.display.flip()
                
#                 # Wait for the AI to "think"
#                 pygame.time.delay(int(thinking_time * 1000))
                
#                 try:
#                     # Get AI's shot
#                     board_x, board_y = ai_opponent.get_shot()  # AI returns board coordinates (row, col)
                    
#                     # Convert from board coordinates to display coordinates for showing to the player
#                     # For display: x = col, y = row
#                     display_x, display_y = board_y, board_x
                    
#                     print(f"AI shot - board coordinates: ({board_x}, {board_y}), display coordinates: ({display_x}, {display_y})")
                    
#                     # Make sure the coordinates are valid (defensive programming)
#                     if not (0 <= board_x < 10 and 0 <= board_y < 10):
#                         # Fallback to a random valid shot if coordinates are invalid
#                         print(f"AI returned invalid shot coordinates: ({board_x}, {board_y})")
#                         valid_shots = [(i, j) for i in range(10) for j in range(10) if (i, j) not in player2_shots]
#                         if valid_shots:
#                             board_x, board_y = random.choice(valid_shots)
#                             display_x, display_y = board_y, board_x  # Convert to display coordinates
#                             board_x, board_y = 0, 0  # Shouldn't happen but is a safe fallback
#                             display_x, display_y = board_y, board_x  # Convert to display coordinates
#                             print("Using emergency fallback coordinates - board: (0, 0), display: (0, 0)")
                    
#                     # Process the shot using board coordinates
#                     # No need to swap here since we're directly working with board coordinates
#                     hit, ship_sunk = False, False
                    
#                     # Add hit to shots set using board coordinates
#                     if (board_x, board_y) not in player2_shots:
#                         player2_shots.add((board_x, board_y))
                        
#                         # Check for hit and ship_sunk
#                         for ship in player1_board.ships:
#                             if ship.receive_hit(board_x, board_y):
#                                 hit = True
#                                 ship_sunk = ship.is_sunk()
#                                 break
                        
#                         # Update board state
#                         if hit:
#                             player1_board.board[board_x, board_y] = CellState.HIT.value
#                             player2_view[board_x][board_y] = CellState.HIT.value  # Use board coordinates
#                             player1_own_view[board_x][board_y] = CellState.HIT.value  # Update player's view
#                         else:
#                             player1_board.board[board_x, board_y] = CellState.MISS.value
#                             player2_view[board_x][board_y] = CellState.MISS.value  # Use board coordinates
#                             player1_own_view[board_x][board_y] = CellState.MISS.value  # Update player's view
                    
#                     # Update AI's knowledge of the shot
#                     ai_opponent.process_shot_result(board_x, board_y, hit, ship_sunk)
                    
#                     # Draw the updated boards to show the AI's shot
#                     screen.fill(BLACK)
#                     # Using display coordinates for cursor position
#                     draw_board(screen, font, player2_view, 150, 80, 30, display_x, display_y, True, "AI's Shot")
#                     draw_board(screen, font, player1_own_view, 400, 80, 25, -1, -1, False, "Your Board")
                    
#                     # Draw status - using display coordinates for showing to the player
#                     hit_text = "HIT!" if hit else "MISS"
#                     ship_text = " Ship sunk!" if ship_sunk else ""
#                     status_text = small_font.render(f"AI fired at {chr(65 + display_x)}{display_y + 1}: {hit_text}{ship_text}", True, 
#                                                 (255, 0, 0) if hit else (255, 255, 255))
#                     screen.blit(status_text, (WIDTH // 2 - 120, HEIGHT - 40))
                    
#                     # Update display to show shot
#                     pygame.display.flip()
                    
#                     # Delay to show the AI's move result
#                     pygame.time.delay(2000)
                    
#                     # Check if game over
#                     new_winner = check_game_over()
#                     if new_winner:
#                         winner = new_winner
#                     else:
#                       transition_screen.show_turn_result(
#                         2,                # AI is player 2
#                         display_y,        # Row coordinate of shot
#                         display_x,        # Column coordinate of shot
#                         hit,
#                         ship_sunk,
#                         True,             # is_ai_mode=True
#                         player1_own_view  # Player's board for preview
#                     )
#                     transition_screen.show_player_ready_screen(
#                         1,                # Next player (human)
#                         True,             # is_ai_mode=True
#                         player1_own_view  # Player's board for preview
#                     )

#                         # Switch back to player's turn
#                     current_player = 1

#                 except Exception as e:
#                     print(f"AI error: {e}")
#                     import traceback
#                     traceback.print_exc()
                    
#                     # Show error message and continue
#                     error_text = small_font.render(f"AI error: {str(e)[:30]}...", True, (255, 0, 0))
#                     screen.blit(error_text, (WIDTH // 2 - 120, HEIGHT - 70))
#                     pygame.display.flip()
#                     pygame.time.delay(2000)
                    
            
#             # Update display
#             pygame.display.flip()
#             clock.tick(30)
            
#         return winner
        
#     except Exception as e:
#         print(f"Error in game_screen: {e}")
#         import traceback
#         traceback.print_exc()
#         return None

# def draw_board(screen, font, board, offset_x, offset_y, cell_size, cursor_x, cursor_y, show_cursor, title=None):
#     """Helper function to draw a game board"""
#     # Draw title if provided
#     if title:
#         title_text = font.render(title, True, WHITE)
#         title_rect = title_text.get_rect(center=(offset_x + (10 * cell_size) // 2, offset_y - 30))
#         screen.blit(title_text, title_rect)
    
#     # Draw column labels (A-J)
#     for i in range(10):
#         letter = chr(65 + i)
#         text = pygame.font.Font(None, 20).render(letter, True, WHITE)
#         screen.blit(text, (offset_x + i * cell_size + cell_size // 3, offset_y - 20))
    
#     # Draw row labels (1-10)
#     for i in range(10):
#         number = str(i + 1)
#         text = pygame.font.Font(None, 20).render(number, True, WHITE)
#         screen.blit(text, (offset_x - 20, offset_y + i * cell_size + cell_size // 3))
    
#     # Draw grid cells
#     for y in range(10):
#         for x in range(10):
#             cell_rect = pygame.Rect(
#                 offset_x + x * cell_size,
#                 offset_y + y * cell_size,
#                 cell_size - 2,
#                 cell_size - 2
#             )
            
#             # Access board using [row][col]
#             cell_value = board[y][x]
            
#             if cell_value == CellState.EMPTY.value:
#                 color = (50, 50, 50)  # Empty cell
#             elif cell_value == CellState.SHIP.value:
#                 color = (0, 255, 0)   # Ship
#             elif cell_value == CellState.HIT.value:
#                 color = (255, 0, 0)   # Hit
#             else:
#                 color = (0, 0, 255)   # Miss
            
#             # Draw cell
#             pygame.draw.rect(screen, color, cell_rect)
#             pygame.draw.rect(screen, (100, 100, 100), cell_rect, 1)
    
#     # Draw cursor if needed
#     if show_cursor and cursor_x >= 0 and cursor_y >= 0:
#         cursor_rect = pygame.Rect(
#             offset_x + cursor_x * cell_size - 2,
#             offset_y + cursor_y * cell_size - 2,
#             cell_size + 2,
#             cell_size + 2
#         )
#         pygame.draw.rect(screen, (255, 255, 0), cursor_rect, 2)

# def game_mode_select():
#     """Screen to select game mode (AI or Player) and AI difficulty"""
#     clock = pygame.time.Clock()
#     font = pygame.font.Font(None, 36)
#     small_font = pygame.font.Font(None, 28)
    
#     # Menu options
#     options = ["VS AI", "VS Player"]
#     ai_difficulties = ["Easy", "Medium", "Hard", "Pao"]  # Added Pao mode
    
#     # Selection state
#     current_option = 0  # 0 for VS AI, 1 for VS Player
#     current_difficulty = 0  # 0 for Easy, 1 for Medium, 2 for Hard, 3 for Pao
#     show_difficulty = False  # Only show difficulty options if AI is selected
    
#     running = True
#     while running:
#         # Fill background
#         screen.fill(BLACK)
        
#         # Draw title
#         title_text = font.render("Select Game Mode", True, WHITE)
#         title_rect = title_text.get_rect(center=(WIDTH // 2, 80))
#         screen.blit(title_text, title_rect)
        
#         # Draw options
#         for i, option in enumerate(options):
#             color = LIGHT_BLUE if i == current_option else WHITE
#             option_text = font.render(option, True, color)
#             option_rect = option_text.get_rect(center=(WIDTH // 2, 180 + i * 60))
#             screen.blit(option_text, option_rect)
            
#             # Draw highlight box around selected option
#             if i == current_option:
#                 rect = pygame.Rect(option_rect.left - 10, option_rect.top - 5, 
#                                   option_rect.width + 20, option_rect.height + 10)
#                 pygame.draw.rect(screen, color, rect, 2, border_radius=5)
        
#         # Draw difficulty options if VS AI is selected
#         if current_option == 0:
#             difficulty_title = small_font.render("Select Difficulty:", True, WHITE)
#             screen.blit(difficulty_title, (WIDTH // 2 - 100, 320))
            
#             for i, difficulty in enumerate(ai_difficulties):
#                 # Special color for Pao mode
#                 if difficulty == "Pao":
#                     color = (255, 0, 0) if i == current_difficulty else (255, 100, 100)
#                 else:
#                     color = LIGHT_BLUE if i == current_difficulty else WHITE
                
#                 diff_text = small_font.render(difficulty, True, color)
#                 diff_rect = diff_text.get_rect(center=(WIDTH // 2, 360 + i * 40))
#                 screen.blit(diff_text, diff_rect)
                
#                 # Draw highlight box around selected difficulty
#                 if i == current_difficulty:
#                     rect = pygame.Rect(diff_rect.left - 10, diff_rect.top - 5, 
#                                       diff_rect.width + 20, diff_rect.height + 10)
#                     pygame.draw.rect(screen, color, rect, 2, border_radius=5)
            
#             # Add warning for Pao mode
#             if current_difficulty == 3:
#                 warning_text = small_font.render("WARNING: Impossible difficulty!", True, (255, 0, 0))
#                 warning_rect = warning_text.get_rect(center=(WIDTH // 2, 520))
#                 screen.blit(warning_text, warning_rect)
        
#         # Draw controls help
#         help_text = small_font.render("Up/Down: Navigate | Fire: Select | Mode: Back", True, LIGHT_GRAY)
#         screen.blit(help_text, (WIDTH // 2 - 190, HEIGHT - 40))
        
#         # Process events
#         for event in pygame.event.get():
#             if event.type == pygame.QUIT:
#                 running = False
#                 pygame.quit()
#                 sys.exit()
                
#             elif event.type == pygame.KEYDOWN:
#                 if event.key == pygame.K_ESCAPE or event.key == pygame.K_TAB:
#                     # Return to main menu
#                     running = False
#                 elif event.key == pygame.K_UP:
#                     if current_option == 0 and current_difficulty > 0:
#                         # Navigate difficulty options
#                         current_difficulty -= 1
#                     else:
#                         # Navigate main options
#                         current_option = (current_option - 1) % len(options)
#                         current_difficulty = 0  # Reset difficulty selection
#                 elif event.key == pygame.K_DOWN:
#                     if current_option == 0 and current_difficulty < len(ai_difficulties) - 1:
#                         # Navigate difficulty options
#                         current_difficulty += 1
#                     else:
#                         # Navigate main options
#                         current_option = (current_option + 1) % len(options)
#                         current_difficulty = 0  # Reset difficulty selection
#                 elif event.key == pygame.K_RETURN or event.key == pygame.K_SPACE:
#                     # Launch ship placement screen with selected options
#                     ai_mode = (current_option == 0)
#                     difficulty = ai_difficulties[current_difficulty] if ai_mode else None
                    
#                     # Create and run the ship placement screen
#                     placement_screen = ShipPlacementScreen(screen, gpio_handler, ai_mode, difficulty)
#                     player1_board, player2_board = placement_screen.run()
                    
#                     # Start the game with the configured boards
#                     game_screen(ai_mode, difficulty, player1_board, player2_board)
#                     running = False  # Exit mode selection after game ends
        
#         # Check GPIO buttons
#         button_states = gpio_handler.get_button_states()
        
#         if button_states['up']:
#             if current_option == 0 and current_difficulty > 0:
#                 # Navigate difficulty options
#                 current_difficulty -= 1
#             else:
#                 # Navigate main options
#                 current_option = (current_option - 1) % len(options)
#                 current_difficulty = 0  # Reset difficulty selection
        
#         if button_states['down']:
#             if current_option == 0 and current_difficulty < len(ai_difficulties) - 1:
#                 # Navigate difficulty options
#                 current_difficulty += 1
#             else:
#                 # Navigate main options
#                 current_option = (current_option + 1) % len(options)
#                 current_difficulty = 0  # Reset difficulty selection
        
#         if button_states['fire']:
#             # Launch ship placement screen with selected options
#             ai_mode = (current_option == 0)
#             difficulty = ai_difficulties[current_difficulty] if ai_mode else None
            
#             # Create and run the ship placement screen
#             placement_screen = ShipPlacementScreen(screen, gpio_handler, ai_mode, difficulty)
#             player1_board, player2_board = placement_screen.run()
            
#             # Start the game with the configured boards
#             game_screen(ai_mode, difficulty, player1_board, player2_board)
#             running = False  # Exit mode selection after game ends
        
#         if button_states['mode']:
#             running = False  # Return to main menu
        
#         # Update display
#         pygame.display.flip()
#         clock.tick(30)

# def main_menu():
#     # Set up menu buttons
#     button_width = 200
#     button_height = 50
#     center_x = (WIDTH - button_width) // 2
#     start_y = 200
#     spacing = 70

#     buttons = [
#         Button(center_x, start_y, button_width, button_height, "Start Game", game_mode_select),
#         Button(center_x, start_y + spacing, button_width, button_height, "Settings", settings_screen),
#         Button(center_x, start_y + spacing * 2, button_width, button_height, "Quit", quit_game)
#     ]
    
#     # Default selection
#     current_selection = 0
#     buttons[current_selection].selected = True

#     clock = pygame.time.Clock()
    
#     # Main menu loop
#     running = True
#     while running:
#         # Fill background
#         screen.fill(BLACK)
        
#         # Draw title
#         title_text = title_font.render("Pao'er Ship", True, WHITE)
#         title_rect = title_text.get_rect(center=(WIDTH // 2, 100))
#         screen.blit(title_text, title_rect)
        
#         # Process events
#         for event in pygame.event.get():
#             if event.type == pygame.QUIT:
#                 running = False
            
#             # Mouse events
#             elif event.type == pygame.MOUSEMOTION:
#                 for button in buttons:
#                     button.check_hover(event.pos)
#             elif event.type == pygame.MOUSEBUTTONDOWN:
#                 for button in buttons:
#                     button.check_click(event.pos)
            
#             # Keyboard navigation
#             elif event.type == pygame.KEYDOWN:
#                 if event.key == pygame.K_UP:
#                     # Clear selections
#                     for button in buttons:
#                         button.selected = False
#                     # Move selection up
#                     current_selection = (current_selection - 1) % len(buttons)
#                     buttons[current_selection].selected = True
                
#                 elif event.key == pygame.K_DOWN:
#                     # Clear selections
#                     for button in buttons:
#                         button.selected = False
#                     # Move selection down
#                     current_selection = (current_selection + 1) % len(buttons)
#                     buttons[current_selection].selected = True
                
#                 elif event.key == pygame.K_RETURN or event.key == pygame.K_SPACE:
#                     # Activate selected button
#                     buttons[current_selection].action()
                
#                 elif event.key == pygame.K_ESCAPE:
#                     running = False
        
#         # Check for GPIO button presses
#         button_states = gpio_handler.get_button_states()
        
#         if button_states['up']:
#             # Clear selections
#             for button in buttons:
#                 button.selected = False
#             # Move selection up
#             current_selection = (current_selection - 1) % len(buttons)
#             buttons[current_selection].selected = True
        
#         if button_states['down']:
#             # Clear selections
#             for button in buttons:
#                 button.selected = False
#             # Move selection down
#             current_selection = (current_selection + 1) % len(buttons)
#             buttons[current_selection].selected = True
        
#         if button_states['fire']:
#             # Activate selected button
#             buttons[current_selection].action()
        
#         # Update and draw buttons
#         for button in buttons:
#             button.update()
#             button.draw(screen)
        
#         # Display controls help
#         help_font = pygame.font.Font(None, 24)
#         help_text = help_font.render("Up/Down: Navigate | Fire: Select | Mode: Back", True, LIGHT_GRAY)
#         screen.blit(help_text, (WIDTH // 2 - 150, HEIGHT - 40))
        
#         # Update display
#         pygame.display.flip()
        
#         # Limit framerate
#         clock.tick(30)

# # Initialize GPIO handler
# gpio_handler = GPIOHandler()

# def main():
#     try:
#         main_menu()
#     except Exception as e:
#         print(f"Error: {e}")
#         import traceback
#         traceback.print_exc()
#     finally:
#         gpio_handler.cleanup()
#         pygame.quit()

# if __name__ == "__main__":
#     main()








"""
Main game script using Pyglet for the Battleship game.
This replaces the original Pygame implementation.
"""
import pyglet
import os
import sys
import time
import random
import numpy as np

# Try to import GPIO support for Raspberry Pi
try:
    import gpiod
    IS_RASPBERRY_PI = True
except ImportError:
    IS_RASPBERRY_PI = False

# Add current directory to path
current_dir = os.path.dirname(os.path.abspath(__file__))
if current_dir not in sys.path:
    sys.path.append(current_dir)

# Import game modules
from src.board.game_board import GameBoard, CellState
from src.game.ai_opponent import AIOpponent, AIDifficulty
from src.utils.constants import SHIP_TYPES

# Import hardware interface
from src.hardware.display_interface import DisplayInterface
from src.hardware.pyglet_display import PygletDisplay

# Import multiple display support
from src.display.multi_display_manager import MultiDisplayManager

# Import input handler
from src.input.pyglet_input_handler import PygletInputHandler

# Import GPIO handlers
if IS_RASPBERRY_PI:
    from src.hardware.gpio_rpi import GPIORPI as GPIOHandler
else:
    from src.hardware.gpio_mock import GPIOMock as GPIOHandler

# Global state
current_screen = None
game_state = None
display_manager = None
gpio_handler = None

# Game constants
SCREEN_WIDTH = 800
SCREEN_HEIGHT = 600

class GameState:
    """Simple class to keep track of game state"""
    def __init__(self):
        self.player1_board = GameBoard()
        self.player2_board = GameBoard()
        self.current_player = 1
        self.game_mode = "menu"  # "menu", "ship_placement", "game", "game_over"
        self.ai_mode = True
        self.difficulty = "Medium"
        self.winner = None
        self.player1_shots = set()
        self.player2_shots = set()

class MenuScreen:
    """Main menu screen"""
    def __init__(self, window):
        self.window = window
        self.batch = pyglet.graphics.Batch()
        
        # Calculate dimensions
        self.width = window.width
        self.height = window.height
        
        # Menu options
        self.options = [
            "Start Game",
            "Settings",
            "Quit"
        ]
        
        self.selected_index = 0
        
        # Create labels
        self._create_labels()
        
        # Create input handler
        self.input_handler = PygletInputHandler(window, gpio_handler)
    
    def _create_labels(self):
        """Create all labels for the menu"""
        # Title
        self.title_label = pyglet.text.Label(
            "Pao'er Ship",
            font_name='Arial',
            font_size=36,
            x=self.width//2,
            y=self.height-100,
            anchor_x='center',
            anchor_y='center',
            color=(255, 255, 255, 255),
            batch=self.batch
        )
        
        # Menu options
        self.option_labels = []
        for i, option in enumerate(self.options):
            label = pyglet.text.Label(
                option,
                font_name='Arial',
                font_size=24,
                x=self.width//2,
                y=self.height-200-(i*50),
                anchor_x='center',
                anchor_y='center',
                color=(200, 200, 200, 255),
                batch=self.batch
            )
            self.option_labels.append(label)
        
        # Help text
        self.help_label = pyglet.text.Label(
            "Up/Down: Navigate | Fire: Select | Mode: Back",
            font_name='Arial',
            font_size=14,
            x=self.width//2,
            y=30,
            anchor_x='center',
            anchor_y='center',
            color=(180, 180, 180, 255),
            batch=self.batch
        )
        
        # Update visuals
        self._update_option_highlights()
    
    def _update_option_highlights(self):
        """Update the visual appearance of menu items"""
        for i, label in enumerate(self.option_labels):
            if i == self.selected_index:
                label.color = (80, 170, 255, 255)  # Highlight color
                label.font_size = 28
            else:
                label.color = (200, 200, 200, 255)  # Normal color
                label.font_size = 24
    
    def update(self, dt):
        """Update menu logic"""
        actions = self.input_handler.update()
        
        if actions['menu_up']:
            self.selected_index = (self.selected_index - 1) % len(self.options)
            self._update_option_highlights()
        
        if actions['menu_down']:
            self.selected_index = (self.selected_index + 1) % len(self.options)
            self._update_option_highlights()
        
        if actions['menu_select']:
            self._handle_selection()
    
    def _handle_selection(self):
        """Handle menu selection"""
        global current_screen, game_state
        
        if self.selected_index == 0:  # Start Game
            # Show game mode selection screen
            current_screen = GameModeScreen(self.window)
        
        elif self.selected_index == 1:  # Settings
            # Show settings screen
            current_screen = SettingsScreen(self.window)
        
        elif self.selected_index == 2:  # Quit
            # Quit the game
            pyglet.app.exit()
    
    def draw(self):
        """Draw the menu screen"""
        self.window.clear()
        self.batch.draw()

class GameModeScreen:
    """Game mode selection screen"""
    def __init__(self, window):
        self.window = window
        self.batch = pyglet.graphics.Batch()
        
        # Calculate dimensions
        self.width = window.width
        self.height = window.height
        
        # Game mode options
        self.modes = ["VS AI", "VS Player"]
        self.difficulties = ["Easy", "Medium", "Hard", "Pao"]
        
        # Selection state
        self.selected_mode = 0
        self.selected_difficulty = 1  # Default to Medium
        self.selecting_difficulty = False
        
        # Create labels
        self._create_labels()
        
        # Create input handler
        self.input_handler = PygletInputHandler(window, gpio_handler)
    
    def _create_labels(self):
        """Create all labels for the mode selection screen"""
        # Title
        self.title_label = pyglet.text.Label(
            "Select Game Mode",
            font_name='Arial',
            font_size=36,
            x=self.width//2,
            y=self.height-100,
            anchor_x='center',
            anchor_y='center',
            color=(255, 255, 255, 255),
            batch=self.batch
        )
        
        # Mode options
        self.mode_labels = []
        for i, mode in enumerate(self.modes):
            label = pyglet.text.Label(
                mode,
                font_name='Arial',
                font_size=24,
                x=self.width//2,
                y=self.height-200-(i*50),
                anchor_x='center',
                anchor_y='center',
                color=(200, 200, 200, 255),
                batch=self.batch
            )
            self.mode_labels.append(label)
        
        # Difficulty label
        self.difficulty_label = pyglet.text.Label(
            "Select Difficulty:",
            font_name='Arial',
            font_size=24,
            x=self.width//2,
            y=self.height-350,
            anchor_x='center',
            anchor_y='center',
            color=(255, 255, 255, 255),
            batch=self.batch
        )
        
        # Difficulty options
        self.difficulty_labels = []
        for i, difficulty in enumerate(self.difficulties):
            # Special handling for Pao mode
            color = (255, 100, 100, 255) if difficulty == "Pao" else (200, 200, 200, 255)
            
            label = pyglet.text.Label(
                difficulty,
                font_name='Arial',
                font_size=20,
                x=self.width//2,
                y=self.height-400-(i*40),
                anchor_x='center',
                anchor_y='center',
                color=color,
                batch=self.batch
            )
            self.difficulty_labels.append(label)
        
        # Warning for Pao mode
        self.pao_warning = pyglet.text.Label(
            "WARNING: Impossible difficulty!",
            font_name='Arial',
            font_size=18,
            x=self.width//2,
            y=90,
            anchor_x='center',
            anchor_y='center',
            color=(255, 0, 0, 255),
            batch=self.batch
        )
        
        # Help text
        self.help_label = pyglet.text.Label(
            "Up/Down: Navigate | Fire: Select | Mode: Back",
            font_name='Arial',
            font_size=14,
            x=self.width//2,
            y=30,
            anchor_x='center',
            anchor_y='center',
            color=(180, 180, 180, 255),
            batch=self.batch
        )
        
        # Update visuals
        self._update_option_highlights()
    
    def _update_option_highlights(self):
        """Update the visual appearance of options"""
        # Update mode labels
        for i, label in enumerate(self.mode_labels):
            if i == self.selected_mode and not self.selecting_difficulty:
                label.color = (80, 170, 255, 255)  # Highlight color
                label.font_size = 28
            else:
                label.color = (200, 200, 200, 255)  # Normal color
                label.font_size = 24
        
        # Update difficulty labels
        for i, label in enumerate(self.difficulty_labels):
            # Special handling for Pao mode
            if self.difficulties[i] == "Pao":
                base_color = (255, 100, 100, 255)
                highlight_color = (255, 0, 0, 255)
            else:
                base_color = (200, 200, 200, 255)
                highlight_color = (80, 170, 255, 255)
            
            if i == self.selected_difficulty and self.selecting_difficulty:
                label.color = highlight_color
                label.font_size = 24
            else:
                label.color = base_color
                label.font_size = 20
        
        # Show/hide difficulty section and warning
        difficulty_visible = (self.selected_mode == 0)
        self.difficulty_label.visible = difficulty_visible
        
        for label in self.difficulty_labels:
            label.visible = difficulty_visible
        
        # Show warning only for Pao mode
        self.pao_warning.visible = (difficulty_visible and self.selected_difficulty == 3)
    
    def update(self, dt):
        """Update mode selection logic"""
        actions = self.input_handler.update()
        
        if actions['menu_up']:
            if self.selecting_difficulty:
                # Navigate difficulty options
                self.selected_difficulty = (self.selected_difficulty - 1) % len(self.difficulties)
            else:
                # Navigate mode options
                self.selected_mode = (self.selected_mode - 1) % len(self.modes)
            
            self._update_option_highlights()
        
        if actions['menu_down']:
            if self.selecting_difficulty:
                # Navigate difficulty options
                self.selected_difficulty = (self.selected_difficulty + 1) % len(self.difficulties)
            else:
                # Navigate mode options
                self.selected_mode = (self.selected_mode + 1) % len(self.modes)
            
            self._update_option_highlights()
        
        if actions['menu_select']:
            if self.selected_mode == 0 and not self.selecting_difficulty:
                # If selected "VS AI", show difficulty selection
                self.selecting_difficulty = True
            else:
                # Start the game with selected options
                self._start_game()
            
            self._update_option_highlights()
        
        if actions['menu_back']:
            if self.selecting_difficulty:
                # Go back to mode selection
                self.selecting_difficulty = False
                self._update_option_highlights()
            else:
                # Go back to main menu
                global current_screen
                current_screen = MenuScreen(self.window)
    
    def _start_game(self):
        """Start the game with selected options"""
        global current_screen, game_state
        
        # Update game state
        game_state.ai_mode = (self.selected_mode == 0)
        game_state.difficulty = self.difficulties[self.selected_difficulty] if game_state.ai_mode else None
        game_state.game_mode = "ship_placement"
        
        # Start ship placement
        current_screen = ShipPlacementScreen(self.window)
    
    def draw(self):
        """Draw the mode selection screen"""
        self.window.clear()
        self.batch.draw()

class SettingsScreen:
    """Settings screen"""
    def __init__(self, window):
        self.window = window
        self.batch = pyglet.graphics.Batch()
        
        # Calculate dimensions
        self.width = window.width
        self.height = window.height
        
        # Create labels
        self.title_label = pyglet.text.Label(
            "Settings",
            font_name='Arial',
            font_size=36,
            x=self.width//2,
            y=self.height-100,
            anchor_x='center',
            anchor_y='center',
            color=(255, 255, 255, 255),
            batch=self.batch
        )
        
        # Info text
        self.info_label = pyglet.text.Label(
            "Settings will be available in a future update.",
            font_name='Arial',
            font_size=20,
            x=self.width//2,
            y=self.height//2,
            anchor_x='center',
            anchor_y='center',
            color=(200, 200, 200, 255),
            batch=self.batch
        )
        
        # Help text
        self.help_label = pyglet.text.Label(
            "Press any button to return to menu",
            font_name='Arial',
            font_size=16,
            x=self.width//2,
            y=self.height//2 - 50,
            anchor_x='center',
            anchor_y='center',
            color=(180, 180, 180, 255),
            batch=self.batch
        )
        
        # Create input handler
        self.input_handler = PygletInputHandler(window, gpio_handler)
    
    def update(self, dt):
        """Update settings screen logic"""
        actions = self.input_handler.update()
        
        # Any button press returns to main menu
        if any(value for key, value in actions.items() if key != 'position'):
            global current_screen
            current_screen = MenuScreen(self.window)
    
    def draw(self):
        """Draw the settings screen"""
        self.window.clear()
        self.batch.draw()

class ShipPlacementScreen:
    """Ship placement screen"""
    def __init__(self, window):
        self.window = window
        self.batch = pyglet.graphics.Batch()
        
        # Calculate dimensions
        self.width = window.width
        self.height = window.height
        
        # Grid settings
        self.cell_size = 30
        self.grid_offset_x = 150
        self.opponent_grid_offset_x = 150
        self.player_grid_offset_x = self.width - 350
        self.grid_offset_y = 100
        
        # Cursor position
        self.cursor_x = 0
        self.cursor_y = 0
        
        # Create UI elements
        self._create_ui_elements()
        
        # Create grid elements
        self.opponent_grid = {}
        self.player_grid = {}
        self._update_grid()
        
        # Create input handler
        self.input_handler = PygletInputHandler(window, gpio_handler)
        
        # Start AI turn if it goes first and AI mode is enabled
        if self.ai_mode and game_state.current_player == 2:
            # Schedule AI move with a delay
            pyglet.clock.schedule_once(self._make_ai_move, 1.0)
    
    def _create_ui_elements(self):
        """Create UI elements for the game screen"""
        # Game mode text
        mode_text = f"VS AI ({self.difficulty})" if self.ai_mode else "VS Player"
        
        self.mode_label = pyglet.text.Label(
            mode_text,
            font_name='Arial',
            font_size=18,
            x=self.width - 100,
            y=self.height - 20,
            anchor_x='center',
            anchor_y='center',
            color=(200, 200, 200, 255),
            batch=self.batch
        )
        
        # Current player
        player_text = "Your Turn" if game_state.current_player == 1 else ("AI's Turn" if self.ai_mode else "Player 2's Turn")
        
        self.player_label = pyglet.text.Label(
            player_text,
            font_name='Arial',
            font_size=24,
            x=20,
            y=self.height - 50,
            anchor_x='left',
            anchor_y='center',
            color=(255, 255, 255, 255),
            batch=self.batch
        )
        
        # Status text
        self.status_label = pyglet.text.Label(
            "Fire at the opponent's board!",
            font_name='Arial',
            font_size=18,
            x=self.width // 2,
            y=40,
            anchor_x='center',
            anchor_y='center',
            color=(200, 200, 200, 255),
            batch=self.batch
        )
        
        # Board labels
        self.opponent_label = pyglet.text.Label(
            "Opponent's Board",
            font_name='Arial',
            font_size=18,
            x=self.opponent_grid_offset_x + 5 * self.cell_size,
            y=self.grid_offset_y + 10 * self.cell_size + 20,
            anchor_x='center',
            anchor_y='center',
            color=(200, 200, 200, 255),
            batch=self.batch
        )
        
        self.player_label = pyglet.text.Label(
            "Your Board",
            font_name='Arial',
            font_size=18,
            x=self.player_grid_offset_x + 5 * self.cell_size,
            y=self.grid_offset_y + 10 * self.cell_size + 20,
            anchor_x='center',
            anchor_y='center',
            color=(200, 200, 200, 255),
            batch=self.batch
        )
        
        # Controls help
        self.controls = [
            pyglet.text.Label(
                "Up/Down/Left/Right: Move cursor",
                font_name='Arial',
                font_size=14,
                x=20,
                y=80,
                anchor_x='left',
                anchor_y='center',
                color=(180, 180, 180, 255),
                batch=self.batch
            ),
            pyglet.text.Label(
                "Fire: Shoot",
                font_name='Arial',
                font_size=14,
                x=20,
                y=60,
                anchor_x='left',
                anchor_y='center',
                color=(180, 180, 180, 255),
                batch=self.batch
            ),
            pyglet.text.Label(
                "Mode: Exit to menu",
                font_name='Arial',
                font_size=14,
                x=20,
                y=40,
                anchor_x='left',
                anchor_y='center',
                color=(180, 180, 180, 255),
                batch=self.batch
            )
        ]
    
    def _update_grid(self):
        """Update both game grids"""
        # Clear previous grid elements
        self.opponent_grid = {}
        self.player_grid = {}
        
        # Update opponent grid (showing player's shots)
        self._update_opponent_grid()
        
        # Update player grid (showing own ships and opponent's shots)
        self._update_player_grid()
    
    def _update_opponent_grid(self):
        """Update the opponent's grid display"""
        # Get opponent board and player shots
        opponent_board = game_state.player2_board
        player_shots = game_state.player1_shots
        
        # Draw grid cells
        for y in range(10):
            for x in range(10):
                cell_x = self.opponent_grid_offset_x + x * self.cell_size
                cell_y = self.grid_offset_y + y * self.cell_size
                
                # Default color (empty)
                color = (50, 50, 50, 255)
                
                # Check if this position has been shot
                if (y, x) in player_shots:
                    # Check if it was a hit
                    if opponent_board.board[y, x] == CellState.SHIP.value:
                        color = (255, 0, 0, 255)  # Hit
                    else:
                        color = (0, 0, 255, 255)  # Miss
                
                # Create cell
                cell = pyglet.shapes.Rectangle(
                    cell_x, cell_y,
                    self.cell_size - 2, self.cell_size - 2,
                    color=color,
                    batch=self.batch
                )
                
                self.opponent_grid[(y, x)] = cell
        
        # Draw cursor if it's player 1's turn
        if game_state.current_player == 1 and not game_state.winner:
            cursor_x = self.opponent_grid_offset_x + self.cursor_x * self.cell_size - 2
            cursor_y = self.grid_offset_y + self.cursor_y * self.cell_size - 2
            
            cursor_outline = pyglet.shapes.BorderedRectangle(
                cursor_x, cursor_y,
                self.cell_size + 2, self.cell_size + 2,
                border=2,
                border_color=(255, 255, 0, 255),
                color=(0, 0, 0, 0),  # Transparent fill
                batch=self.batch
            )
            
            self.opponent_grid['cursor'] = cursor_outline
    
    def _update_player_grid(self):
        """Update the player's grid display"""
        # Get player board and opponent shots
        player_board = game_state.player1_board
        opponent_shots = game_state.player2_shots
        
        # Draw grid cells
        for y in range(10):
            for x in range(10):
                cell_x = self.player_grid_offset_x + x * self.cell_size
                cell_y = self.grid_offset_y + y * self.cell_size
                
                # Get cell state
                cell_state = player_board.board[y, x]
                
                # Check if this position has been shot
                is_hit = (y, x) in opponent_shots and cell_state == CellState.SHIP.value
                is_miss = (y, x) in opponent_shots and cell_state == CellState.EMPTY.value
                
                # Determine color
                if is_hit:
                    color = (255, 0, 0, 255)  # Hit ship
                elif is_miss:
                    color = (0, 0, 255, 255)  # Miss
                elif cell_state == CellState.SHIP.value:
                    color = (0, 255, 0, 255)  # Undamaged ship
                else:
                    color = (50, 50, 50, 255)  # Empty
                
                # Create cell
                cell = pyglet.shapes.Rectangle(
                    cell_x, cell_y,
                    self.cell_size - 2, self.cell_size - 2,
                    color=color,
                    batch=self.batch
                )
                
                self.player_grid[(y, x)] = cell
    
    def _process_shot(self, x, y):
        """Process a shot at coordinates (x,y)"""
        # Determine target based on current player
        if game_state.current_player == 1:
            # Player 1 firing at Player 2's board
            target_board = game_state.player2_board
            shots = game_state.player1_shots
            target_position = (self.cursor_y, self.cursor_x)  # Note coordinate swap
        else:
            # Player 2 (or AI) firing at Player 1's board
            target_board = game_state.player1_board
            shots = game_state.player2_shots
            target_position = (y, x)  # AI coordinates
        
        # Check if already shot at this position
        if target_position in shots:
            return False, False
        
        # Add to shots history
        shots.add(target_position)
        
        # Check for hit and sunken ship
        hit = False
        ship_sunk = False
        
        for ship in target_board.ships:
            if ship.receive_hit(target_position[0], target_position[1]):
                hit = True
                ship_sunk = ship.is_sunk()
                break
        
        # Update status
        if game_state.current_player == 1:
            self.status_label.text = f"You fired at {chr(65 + x)}{y + 1}: {'HIT!' if hit else 'MISS'}"
        else:
            self.status_label.text = f"Opponent fired at {chr(65 + x)}{y + 1}: {'HIT!' if hit else 'MISS'}"
        
        # Check for win
        if self._check_win():
            return hit, ship_sunk
        
        # Update grid
        self._update_grid()
        
        # Switch player after a short delay (unless hit)
        if not hit:
            pyglet.clock.schedule_once(self._switch_player, 1.0)
        else:
            # If AI and hit, schedule another AI move
            if self.ai_mode and game_state.current_player == 2:
                pyglet.clock.schedule_once(self._make_ai_move, 1.0)
        
        return hit, ship_sunk
    
    def _check_win(self):
        """Check if the current player has won"""
        if game_state.current_player == 1:
            # Check if all player 2's ships are sunk
            target_board = game_state.player2_board
        else:
            # Check if all player 1's ships are sunk
            target_board = game_state.player1_board
        
        # Win if all ships are sunk
        if all(ship.is_sunk() for ship in target_board.ships):
            game_state.winner = game_state.current_player
            self._show_winner()
            return True
        
        return False
    
    def _show_winner(self):
        """Display the winner message"""
        # Update player label with winner
        if game_state.winner == 1:
            self.player_label.text = "YOU WIN!"
            self.player_label.color = (0, 255, 0, 255)
        else:
            if self.ai_mode:
                self.player_label.text = "AI WINS!"
            else:
                self.player_label.text = "PLAYER 2 WINS!"
            self.player_label.color = (255, 0, 0, 255)
        
        # Update status with return to menu message
        self.status_label.text = "Press Mode to return to menu"
    
    def _switch_player(self, dt=0):
        """Switch to the other player's turn"""
        if game_state.winner:
            return
            
        # Toggle player
        game_state.current_player = 3 - game_state.current_player
        
        # Update player label
        if game_state.current_player == 1:
            self.player_label.text = "Your Turn"
        else:
            self.player_label.text = "AI's Turn" if self.ai_mode else "Player 2's Turn"
        
        # Update status
        self.status_label.text = "Fire at the opponent's board!"
        
        # Reset cursor position for player 1
        if game_state.current_player == 1:
            self.cursor_x = 0
            self.cursor_y = 0
        
        # Update grid
        self._update_grid()
        
        # If AI's turn, make AI move
        if self.ai_mode and game_state.current_player == 2:
            pyglet.clock.schedule_once(self._make_ai_move, 1.0)
    
    def _make_ai_move(self, dt=0):
        """Make an AI move"""
        if not self.ai_opponent or game_state.winner:
            return
            
        # Get AI's shot coordinates
        x, y = self.ai_opponent.get_shot()
        
        # Process the shot
        hit, ship_sunk = self._process_shot(x, y)
        
        # Update AI's knowledge
        self.ai_opponent.process_shot_result(y, x, hit, ship_sunk)
    
    def update(self, dt):
        """Update game logic"""
        actions = self.input_handler.update()
        
        # Handle exit to menu
        if actions['menu_back']:
            global current_screen
            current_screen = MenuScreen(self.window)
            return
        
        # Skip if game over
        if game_state.winner:
            if actions['menu_select']:
                # Return to menu when fire is pressed after game ends
                global current_screen
                current_screen = MenuScreen(self.window)
            return
        
        # Only process player 1's input during their turn
        if game_state.current_player == 1:
            # Handle movement
            moved = False
            
            if actions['menu_up'] and self.cursor_y > 0:
                self.cursor_y -= 1
                moved = True
            
            if actions['menu_down'] and self.cursor_y < 9:
                self.cursor_y += 1
                moved = True
            
            if actions['left'] and self.cursor_x > 0:
                self.cursor_x -= 1
                moved = True
            
            if actions['right'] and self.cursor_x < 9:
                self.cursor_x += 1
                moved = True
            
            # Update grid if moved
            if moved:
                self._update_grid()
            
            # Handle fire
            if actions['menu_select']:
                # Skip if already shot at this position
                target_position = (self.cursor_y, self.cursor_x)
                if target_position not in game_state.player1_shots:
                    self._process_shot(self.cursor_x, self.cursor_y)
    
    def draw(self):
        """Draw the game screen"""
        self.window.clear()
        
        # Draw coordinate labels for opponent grid
        for i in range(10):
            # Column labels (A-J)
            label = pyglet.text.Label(
                chr(65 + i),
                font_name='Arial',
                font_size=14,
                x=self.opponent_grid_offset_x + i * self.cell_size + 15,
                y=self.grid_offset_y - 20,
                anchor_x='center',
                anchor_y='center',
                color=(200, 200, 200, 255)
            )
            label.draw()
            
            # Row labels (1-10)
            label = pyglet.text.Label(
                str(i + 1),
                font_name='Arial',
                font_size=14,
                x=self.opponent_grid_offset_x - 20,
                y=self.grid_offset_y + i * self.cell_size + 15,
                anchor_x='center',
                anchor_y='center',
                color=(200, 200, 200, 255)
            )
            label.draw()
        
        # Draw coordinate labels for player grid
        for i in range(10):
            # Column labels (A-J)
            label = pyglet.text.Label(
                chr(65 + i),
                font_name='Arial',
                font_size=14,
                x=self.player_grid_offset_x + i * self.cell_size + 15,
                y=self.grid_offset_y - 20,
                anchor_x='center',
                anchor_y='center',
                color=(200, 200, 200, 255)
            )
            label.draw()
            
            # Row labels (1-10)
            label = pyglet.text.Label(
                str(i + 1),
                font_name='Arial',
                font_size=14,
                x=self.player_grid_offset_x - 20,
                y=self.grid_offset_y + i * self.cell_size + 15,
                anchor_x='center',
                anchor_y='center',
                color=(200, 200, 200, 255)
            )
            label.draw()
        
        # Draw batch (grids, labels, etc.)
        self.batch.draw()


def setup_display():
    """Set up display for the game"""
    global display_manager
    
    # Create display manager
    display_manager = MultiDisplayManager()
    
    # Detect displays
    displays = display_manager.detect_displays()
    
    # Get optimal configuration
    config = display_manager.get_optimal_configuration()
    
    # Create main window
    window = pyglet.window.Window(
        width=config['game_width'],
        height=config['game_height'],
        caption="Pao'er Ship",
        fullscreen=config['fullscreen']
    )
    
    # Fix display issues on Raspberry Pi if needed
    if display_manager.is_rpi:
        display_manager.fix_rpi_display_issues()
    
    return window


def main():
    """Main entry point for the game"""
    global game_state, current_screen, gpio_handler
    
    # Create game state
    game_state = GameState()
    
    # Set up GPIO handler
    if IS_RASPBERRY_PI:
        gpio_handler = GPIOHandler()
    else:
        gpio_handler = GPIOMock()
    
    # Set up display
    window = setup_display()
    
    # Create initial screen
    current_screen = MenuScreen(window)
    
    # Set up update function
    def update(dt):
        if current_screen:
            current_screen.update(dt)
    
    # Set up draw function
    @window.event
    def on_draw():
        if current_screen:
            current_screen.draw()
    
    # Schedule updates
    pyglet.clock.schedule_interval(update, 1/60.0)
    
    # Run the game
    try:
        pyglet.app.run()
    finally:
        # Clean up GPIO resources
        if gpio_handler:
            gpio_handler.cleanup()

if __name__ == "__main__":
    main()

        self.grid_offset_y = 100
        
        # Get game state
        global game_state
        self.ai_mode = game_state.ai_mode
        self.difficulty = game_state.difficulty
        
        # Ship placement state
        self.ship_types = list(SHIP_TYPES.items())
        self.current_ship_index = 0
        self.current_ship_horizontal = True
        
        # Current player (1 or 2)
        self.current_player = 1
        
        # Cursor position
        self.cursor_x = 0
        self.cursor_y = 0
        
        # Placement state
        self.placement_complete = False
        self.placement_valid = True
        
        # Confirmation dialog
        self.showing_confirmation = False
        self.confirmation_option = 0  # 0 = Continue, 1 = Reset
        self.dialog_elements = []
        
        # Create UI elements
        self._create_ui_elements()
        
        # Create grid elements
        self.grid_cells = {}
        self._update_grid()
        
        # Create input handler
        self.input_handler = PygletInputHandler(window, gpio_handler)
        
        # Place AI ships if in AI mode
        if self.ai_mode:
            self._place_ai_ships()
    
    def _create_ui_elements(self):
        """Create UI elements for the ship placement screen"""
        # Title
        self.title_label = pyglet.text.Label(
            "Place Your Ships",
            font_name='Arial',
            font_size=36,
            x=self.width//2,
            y=self.height-40,
            anchor_x='center',
            anchor_y='center',
            color=(255, 255, 255, 255),
            batch=self.batch
        )
        
        # Ship info
        ship_name, ship_length = self.ship_types[self.current_ship_index]
        orientation = "Horizontal" if self.current_ship_horizontal else "Vertical"
        
        self.ship_info = pyglet.text.Label(
            f"Placing: {ship_name} ({ship_length}) - {orientation}",
            font_name='Arial',
            font_size=20,
            x=self.grid_offset_x,
            y=self.grid_offset_y - 40,
            anchor_x='left',
            anchor_y='center',
            color=(255, 255, 255, 255),
            batch=self.batch
        )
        
        # Ship list
        self.ship_labels = []
        for i, (ship_name, ship_length) in enumerate(self.ship_types):
            status = "►" if i == self.current_ship_index else "○"
            color = (255, 255, 0, 255) if i == self.current_ship_index else (200, 200, 200, 255)
            
            label = pyglet.text.Label(
                f"{status} {ship_name} ({ship_length})",
                font_name='Arial',
                font_size=16,
                x=self.width - 200,
                y=self.height - 120 - (i * 30),
                anchor_x='left',
                anchor_y='center',
                color=color,
                batch=self.batch
            )
            self.ship_labels.append(label)
        
        # Controls help
        self.controls = [
            pyglet.text.Label(
                "Up/Down/Left/Right: Move",
                font_name='Arial',
                font_size=14,
                x=20,
                y=80,
                anchor_x='left',
                anchor_y='center',
                color=(180, 180, 180, 255),
                batch=self.batch
            ),
            pyglet.text.Label(
                "Fire: Place Ship",
                font_name='Arial',
                font_size=14,
                x=20,
                y=60,
                anchor_x='left',
                anchor_y='center',
                color=(180, 180, 180, 255),
                batch=self.batch
            ),
            pyglet.text.Label(
                "Mode: Reset",
                font_name='Arial',
                font_size=14,
                x=20,
                y=40,
                anchor_x='left',
                anchor_y='center',
                color=(180, 180, 180, 255),
                batch=self.batch
            ),
            pyglet.text.Label(
                "R: Rotate Ship",
                font_name='Arial',
                font_size=14,
                x=20,
                y=20,
                anchor_x='left',
                anchor_y='center',
                color=(180, 180, 180, 255),
                batch=self.batch
            )
        ]
    
    def _update_grid(self):
        """Update the grid display"""
        # Clear previous grid elements
        self.grid_cells = {}
        
        # Get the current board
        board = game_state.player1_board if self.current_player == 1 else game_state.player2_board
        
        # Draw grid cells
        for y in range(10):
            for x in range(10):
                cell_x = self.grid_offset_x + x * self.cell_size
                cell_y = self.grid_offset_y + y * self.cell_size
                
                # Get cell state
                cell_state = board.board[y, x]
                
                # Determine color
                if cell_state == CellState.EMPTY.value:
                    color = (50, 50, 50, 255)  # Empty cell
                elif cell_state == CellState.SHIP.value:
                    color = (0, 255, 0, 255)  # Ship
                else:
                    color = (100, 100, 100, 255)  # Other state
                
                # Create cell
                cell = pyglet.shapes.Rectangle(
                    cell_x, cell_y,
                    self.cell_size - 2, self.cell_size - 2,
                    color=color,
                    batch=self.batch
                )
                
                self.grid_cells[(y, x)] = cell
        
        # Draw preview if placing a ship
        if not self.placement_complete and self.current_ship_index < len(self.ship_types):
            ship_name, ship_length = self.ship_types[self.current_ship_index]
            
            # Check if placement is valid
            board = game_state.player1_board if self.current_player == 1 else game_state.player2_board
            self.placement_valid = self._can_place_ship(
                board, self.cursor_x, self.cursor_y, ship_length, self.current_ship_horizontal
            )
            
            # Draw ship preview
            preview_color = (0, 255, 0, 255) if self.placement_valid else (255, 100, 100, 255)
            
            # Draw each segment of the ship
            for i in range(ship_length):
                preview_x = self.cursor_x
                preview_y = self.cursor_y
                
                if self.current_ship_horizontal:
                    preview_x += i
                else:
                    preview_y += i
                
                # Skip if out of bounds
                if preview_x >= 10 or preview_y >= 10:
                    continue
                
                # Calculate position
                cell_x = self.grid_offset_x + preview_x * self.cell_size
                cell_y = self.grid_offset_y + preview_y * self.cell_size
                
                # Create preview cell
                preview = pyglet.shapes.Rectangle(
                    cell_x, cell_y,
                    self.cell_size - 2, self.cell_size - 2,
                    color=preview_color,
                    batch=self.batch
                )
                
                self.grid_cells[('preview', i)] = preview
            
            # Draw cursor highlight
            cursor_x = self.grid_offset_x + self.cursor_x * self.cell_size - 2
            cursor_y = self.grid_offset_y + self.cursor_y * self.cell_size - 2
            
            # Create highlight rectangle
            cursor = pyglet.shapes.Rectangle(
                cursor_x, cursor_y,
                self.cell_size + 2, self.cell_size + 2,
                color=(0, 0, 0, 0),  # Transparent fill
                batch=self.batch
            )
            
            # Create outline for cursor
            cursor_outline = pyglet.shapes.BorderedRectangle(
                cursor_x, cursor_y,
                self.cell_size + 2, self.cell_size + 2,
                border=2,
                border_color=(255, 255, 0, 255),
                color=(0, 0, 0, 0),  # Transparent fill
                batch=self.batch
            )
            
            self.grid_cells['cursor'] = cursor
            self.grid_cells['cursor_outline'] = cursor_outline
    
    def _update_ship_info(self):
        """Update the ship information display"""
        if self.current_ship_index < len(self.ship_types):
            ship_name, ship_length = self.ship_types[self.current_ship_index]
            orientation = "Horizontal" if self.current_ship_horizontal else "Vertical"
            self.ship_info.text = f"Placing: {ship_name} ({ship_length}) - {orientation}"
        else:
            self.ship_info.text = "All ships placed!"
        
        # Update ship labels
        for i, (ship_name, ship_length) in enumerate(self.ship_types):
            if i < self.current_ship_index:
                status = "✓"  # Placed
                color = (0, 255, 0, 255)
            elif i == self.current_ship_index:
                status = "►"  # Current
                color = (255, 255, 0, 255)
            else:
                status = "○"  # Pending
                color = (200, 200, 200, 255)
                
            self.ship_labels[i].text = f"{status} {ship_name} ({ship_length})"
            self.ship_labels[i].color = color
    
    def _can_place_ship(self, board, x, y, length, horizontal):
        """Check if a ship can be placed at the given position"""
        # Check board limits
        if horizontal:
            if x + length > 10:
                return False
        else:
            if y + length > 10:
                return False
                
        # Check for overlap with existing ships
        for i in range(length):
            check_x = x + (i if horizontal else 0)
            check_y = y + (0 if horizontal else i)
                
            if board.board[check_y, check_x] != CellState.EMPTY.value:
                return False
                
        return True
    
    def _place_ship(self):
        """Place the current ship on the board"""
        if self.current_ship_index >= len(self.ship_types):
            return False
        
        ship_name, ship_length = self.ship_types[self.current_ship_index]
        board = game_state.player1_board if self.current_player == 1 else game_state.player2_board
        
        # Place the ship
        success = board.place_ship(
            self.cursor_y,  # Note the coordinate swap to match backend
            self.cursor_x,
            ship_length,
            self.current_ship_horizontal
        )
        
        if success:
            self.current_ship_index += 1
            
            # Reset cursor position for next ship
            self.cursor_x = 0
            self.cursor_y = 0
            
            # Update display
            self._update_ship_info()
            self._update_grid()
            
            # Check if all ships have been placed
            if self.current_ship_index >= len(self.ship_types):
                self.placement_complete = True
                
                # Move to game screen after a delay
                def start_game(dt):
                    global current_screen, game_state
                    game_state.game_mode = "game"
                    current_screen = GameScreen(self.window)
                
                pyglet.clock.schedule_once(start_game, 1.0)
        
        return success
    
    def _place_ai_ships(self):
        """Place ships for AI opponent randomly"""
        # Reset the board first
        game_state.player2_board.reset_board()
        
        # Place each ship
        for ship_name, ship_length in self.ship_types:
            placed = False
            attempts = 0
            
            while not placed and attempts < 100:
                # Get random position and orientation
                x = random.randint(0, 9)
                y = random.randint(0, 9)
                horizontal = random.choice([True, False])
                
                # Try to place
                placed = game_state.player2_board.place_ship(y, x, ship_length, horizontal)
                attempts += 1
            
            # If failed after 100 attempts, try again with all ships
            if not placed:
                game_state.player2_board.reset_board()
                return self._place_ai_ships()
        
        return True
    
    def _reset_placement(self):
        """Reset the ship placement"""
        # Reset the board
        if self.current_player == 1:
            game_state.player1_board.reset_board()
        else:
            game_state.player2_board.reset_board()
        
        # Reset placement state
        self.current_ship_index = 0
        self.current_ship_horizontal = True
        self.cursor_x = 0
        self.cursor_y = 0
        
        # Update display
        self._update_ship_info()
        self._update_grid()
    
    def _show_confirmation_dialog(self):
        """Show the reset confirmation dialog"""
        self.showing_confirmation = True
        self.confirmation_option = 0
        
        # Create dialog elements
        dialog_bg = pyglet.shapes.Rectangle(
            self.width//2 - 150, self.height//2 - 100,
            300, 200,
            color=(50, 50, 50, 255),
            batch=self.batch
        )
        
        dialog_border = pyglet.shapes.BorderedRectangle(
            self.width//2 - 150, self.height//2 - 100,
            300, 200,
            border=2,
            border_color=(255, 255, 255, 255),
            color=(50, 50, 50, 255),
            batch=self.batch
        )
        
        dialog_title = pyglet.text.Label(
            "Reset Placement?",
            font_name='Arial',
            font_size=24,
            x=self.width//2,
            y=self.height//2 + 60,
            anchor_x='center',
            anchor_y='center',
            color=(255, 255, 255, 255),
            batch=self.batch
        )
        
        dialog_msg = pyglet.text.Label(
            "All ships will be removed.",
            font_name='Arial',
            font_size=16,
            x=self.width//2,
            y=self.height//2 + 20,
            anchor_x='center',
            anchor_y='center',
            color=(200, 200, 200, 255),
            batch=self.batch
        )
        
        continue_option = pyglet.text.Label(
            "Continue Placement",
            font_name='Arial',
            font_size=18,
            x=self.width//2,
            y=self.height//2 - 20,
            anchor_x='center',
            anchor_y='center',
            color=(255, 255, 0, 255),  # Highlighted by default
            batch=self.batch
        )
        
        reset_option = pyglet.text.Label(
            "Reset All Ships",
            font_name='Arial',
            font_size=18,
            x=self.width//2,
            y=self.height//2 - 60,
            anchor_x='center',
            anchor_y='center',
            color=(255, 255, 255, 255),
            batch=self.batch
        )
        
        # Store elements
        self.dialog_elements = [
            dialog_bg,
            dialog_border,
            dialog_title,
            dialog_msg,
            continue_option,
            reset_option
        ]
    
    def _hide_confirmation_dialog(self):
        """Hide the reset confirmation dialog"""
        self.showing_confirmation = False
        self.dialog_elements = []
    
    def _update_confirmation_dialog(self):
        """Update the confirmation dialog option highlighting"""
        if not self.dialog_elements:
            return
            
        # Update option colors
        continue_option = self.dialog_elements[4]
        reset_option = self.dialog_elements[5]
        
        continue_option.color = (255, 255, 0, 255) if self.confirmation_option == 0 else (255, 255, 255, 255)
        reset_option.color = (255, 255, 0, 255) if self.confirmation_option == 1 else (255, 255, 255, 255)
    
    def update(self, dt):
        """Update ship placement logic"""
        actions = self.input_handler.update()
        
        # Handle confirmation dialog if showing
        if self.showing_confirmation:
            if actions['menu_up'] or actions['menu_down']:
                self.confirmation_option = 1 - self.confirmation_option
                self._update_confirmation_dialog()
            
            elif actions['menu_select']:
                if self.confirmation_option == 0:  # Continue
                    self._hide_confirmation_dialog()
                else:  # Reset
                    self._hide_confirmation_dialog()
                    self._reset_placement()
            
            return
        
        # Skip if all ships placed
        if self.placement_complete:
            return
        
        # Handle movement
        moved = False
        
        if actions['menu_up'] and self.cursor_y > 0:
            self.cursor_y -= 1
            moved = True
        
        if actions['menu_down'] and self.cursor_y < 9:
            self.cursor_y += 1
            moved = True
        
        if actions['left'] and self.cursor_x > 0:
            self.cursor_x -= 1
            moved = True
        
        if actions['right'] and self.cursor_x < 9:
            self.cursor_x += 1
            moved = True
        
        # Handle rotation
        if actions['rotate']:
            self.current_ship_horizontal = not self.current_ship_horizontal
            moved = True
            self._update_ship_info()
        
        # Update grid if moved
        if moved:
            self._update_grid()
        
        # Handle ship placement
        if actions['menu_select']:
            if self.placement_valid:
                self._place_ship()
        
        # Handle reset
        if actions['menu_back']:
            self._show_confirmation_dialog()
    
    def draw(self):
        """Draw the ship placement screen"""
        self.window.clear()
        
        # Draw column labels
        for i in range(10):
            label = pyglet.text.Label(
                chr(65 + i),
                font_name='Arial',
                font_size=14,
                x=self.grid_offset_x + i * self.cell_size + 15,
                y=self.grid_offset_y - 20,
                anchor_x='center',
                anchor_y='center',
                color=(200, 200, 200, 255)
            )
            label.draw()
        
        # Draw row labels
        for i in range(10):
            label = pyglet.text.Label(
                str(i + 1),
                font_name='Arial',
                font_size=14,
                x=self.grid_offset_x - 20,
                y=self.grid_offset_y + i * self.cell_size + 15,
                anchor_x='center',
                anchor_y='center',
                color=(200, 200, 200, 255)
            )
            label.draw()
        
        # Draw batch (grid, ships, labels, etc.)
        self.batch.draw()

class GameScreen:
    """Main game screen"""
    def __init__(self, window):
        self.window = window
        self.batch = pyglet.graphics.Batch()
        
        # Calculate dimensions
        self.width = window.width
        self.height = window.height
        
        # Get game state
        global game_state
        self.ai_mode = game_state.ai_mode
        self.difficulty = game_state.difficulty
        
        # Initialize AI if needed
        self.ai_opponent = None
        if self.ai_mode:
            difficulty_map = {
                "Easy": AIDifficulty.EASY,
                "Medium": AIDifficulty.MEDIUM,
                "Hard": AIDifficulty.HARD,
                "Pao": AIDifficulty.PAO
            }
            ai_difficulty = difficulty_map.get(self.difficulty, AIDifficulty.MEDIUM)
            self.ai_opponent = AIOpponent(ai_difficulty, game_state.player1_board)
        
        # Grid settings
        self.cell_size = 30
        self.grid_offset_x = 150