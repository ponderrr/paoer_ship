...
def game_screen(ai_mode=True, difficulty="Medium", player1_board=None, player2_board=None):
    ...
    def process_shot(x, y, shooter_board, target_board, shots_set):
        ...
        # FIX: Swap x and y for consistent coordinate mapping
        # In the display, x is column and y is row
        # In the internal board, x is row and y is column
        board_y, board_x = x, y  # Swap x and y to fix coordinate mapping
        
        if (board_y, board_x) in shots_set:
            return False, False  # Shot already taken, no ship sunk
            
        shots_set.add((board_y, board_x))
        ...
        for ship in target_board.ships:
            if ship.receive_hit(board_y, board_x):
                hit = True
    ...
    while running:
        ...
        if ai_mode and current_player == 2 and not winner:
            ...
            y, x = ai_opponent.get_shot()  # FIX: Get shot returns swapped coordinates
            ...
            board_y, board_x = x, y
            if hit:
                player2_view[board_y][board_x] = CellState.HIT.value
                player1_own_view[board_y][board_x] = CellState.HIT.value  # Mark hit on player's view of their board
            else:
                player2_view[board_y][board_x] = CellState.MISS.value
                player1_own_view[board_y][board_x] = CellState.MISS.value  # Mark miss on player's view of their board
            
            # Update AI's knowledge of the shot
            ai_opponent.process_shot_result(y, x, hit, ship_sunk)  # FIX: Uses swapped coordinates
            ...
            # FIX: Correct coordinate display
            human_coords = f"{chr(65 + board_x)}{board_y + 1}"
